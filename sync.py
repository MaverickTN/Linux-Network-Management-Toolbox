...
def get_iptaccount_counters() -> Dict[str, Dict[str, int]]:
    all_counters, zone_map = {}, parse_shorewall_interfaces()
    if not zone_map: return {}
    ZONES_TO_IGNORE = {'net', 'local', 'docker', 'fw'}
    for zone_name in zone_map.keys():
        if zone_name in ZONES_TO_IGNORE: continue
        try:
            ...
            for match in pattern.finditer(result.stdout):
                ...
                all_counters[ip]['in'] += int(bytes_in); all_counters[ip]['out'] += int(bytes_out)
...
def main_loop():
    ...
    while True:
        try:
            ...
            if time.time() - last_purge_time > 86400: print("Running daily database maintenance..."); setup_database(config); last_purge_time = time.time()
            current_counters = get_iptaccount_counters()
            if current_counters:
                current_timestamp = int(time.time())
                paths = config.get("system_paths", {}); leases_file = paths.get("dnsmasq_leases_file", "")
                if not leases_file: ip_to_mac_map = {}
                else: ip_to_mac_map = {l['ip']: l['mac'].replace(':', '') for l in get_active_leases(leases_file)}
                records = []
                ...
                if records:
                    conn = sqlite3.connect(DB_FILE); insert_query = "INSERT INTO bandwidth VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(timestamp, host_id) DO UPDATE SET bytes_in=excluded.bytes_in, bytes_out=excluded.bytes_out, rate_in=excluded.rate_in, rate_out=excluded.rate_out"; conn.executemany(insert_query, records); conn.commit(); conn.close()
