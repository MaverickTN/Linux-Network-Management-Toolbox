...
def remove_expired_blocks(mac):
    blocks = list_host_schedules(mac)
    now = datetime.now()
    valid_blocks = []
    for block in blocks:
        try:
            end_time = datetime.strptime(block["end"], "%Y-%m-%d %H:%M:%S")
            if end_time > now:
                valid_blocks.append(block)
        except Exception as e:
            print(f"Invalid block format for {mac}: {block}")
    schedules = load_schedules()
    schedules[mac.lower()] = valid_blocks
    save_schedules(schedules)
