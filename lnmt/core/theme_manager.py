...
def theme_css_vars(theme_key="dark"):
    ...
    keys = ["primary", "background", "foreground", "accent", "danger", "success", "warning", "info", "border-radius"]
    css = ":root {\n" + "\n".join([f"  --color-{k}: {theme[k]};" for k in keys if k in theme]) + "\n}"
    return css

def cli_color(text, style="primary", theme_key="dark"):
    return f"{THEMES.get(theme_key, THEMES['dark'])['cli'].get(style, '')}{text}{THEMES.get(theme_key, THEMES['dark'])['cli']['reset']}"
