...
def theme_css_vars(theme_key="dark"):
    ...
    css_vars = {
        ...
        "background": theme.get("background", "#23272e"),
        "foreground": theme.get("foreground", "#e0e0e0"),
        "accent": theme.get("accent", "#f39c12"),
        "danger": theme.get("danger", "#e74c3c"),
        "success": theme.get("success", "#43a047"),
        "warning": theme.get("warning", "#ff9800"),
        "info": theme.get("info", "#007bff"),
        "border-radius": theme.get("border-radius", "10px"),
    }
    return ":root {\n" + "\n".join([f"  --color-{k}: {v};" for k, v in css_vars.items()]) + "\n}"

def cli_color(text, style="primary", theme_key="dark"):
    cli_colors = THEMES.get(theme_key, THEMES["dark"])["cli"]
    return f"{cli_colors.get(style, '')}{text}{cli_colors['reset']}"
