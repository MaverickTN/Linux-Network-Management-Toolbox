...
```---
**File to Replace: `inetctl/cli/schedule.py`**
*(Updated to integrate calls to the new logging system.)*
```python
import typer
import json
...
def set_schedule(
    ...
    if not host:
        ...
        if not leases_file: raise typer.BadParameter("dnsmasq_leases_file not defined in config")
