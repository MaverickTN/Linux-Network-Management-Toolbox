...
def home():
    ...
    for mac, host_config in known_hosts_map.items():
        ...
        device.update({"is_online": is_online, "assignment_status": "Reservation", "ip": (lease['ip'] if lease else static_ip), "hostname": (lease.get('hostname') if lease and lease.get('hostname') != '(unknown)' else host_config.get('hostname'))})
    ...
    for device in devices:
        ...
        bucket.setdefault(vlan_id, []).append(device)
...
def get_live_online_status():
    ...
    for mac in all_known_macs:
        ...
        is_online = (mac in {l['mac'] for l in active_leases}) or (static_ip and ping_status_map.get(static_ip, False)) or mac_sanitized in recent_traffic_macs
