...
# --- All Flask-Login, Decorators, and Helper functions are correct and unchanged ---
...
def home():
    config = load_config()
    # --- FIX: Normalize all MAC addresses to lowercase for reliable matching ---
    ...
    active_macs = {lease['mac'] for lease in active_leases} # Lease MACs are already lowercase
    ...
    for mac, host_config in known_hosts_map.items():
        ...
        lease = next((l for l in active_leases if l['mac'] == mac), {}) # Safe lookup
    ...
    all_vlan_keys = sorted([k for k in all_vlan_ids if k in network_map and (active_by_vlan.get(k) or offline_by_vlan.get(k))])
    if 'unassigned' in all_vlan_keys: all_vlan_keys.remove('unassigned'); all_vlan_keys.append('unassigned')
...
# --- All other API and data routes are unchanged and included for completeness ---
@app.route('/logs')
@login_required
