...
# --- All boilerplate and auth routes are unchanged and complete ---
...
def home():
    ...
    for mac, host_config in known_hosts_map.items():
        ...
        is_online = mac in {l['mac'] for l in active_leases} or online_status_map.get(host_config.get("ip_assignment", {}).get("ip"), False)
        ...
        if is_online: active_devices.append(device_data)
        else: offline_reservations.append(device_data)
    ...
    active_by_vlan, offline_by_vlan = {k: [] for k in network_map.keys()}, {k: [] for k in network_map.keys()}
    ...
    all_vlan_keys = sorted([k for k,v in network_map.items() if active_by_vlan.get(k) or offline_by_vlan.get(k)], key=lambda x: (x == 'unassigned', x))
...
# --- All other routes are complete and unchanged from the last working version ---
@app.route('/network')
@login_required
...
def submit_job():
    ...
    if job_type in ["netplan:apply", "netplan:add_interface", "netplan:delete_interface", "api:vlan_toggle_access"] and current_user.role != 'admin': return jsonify({"status": "error", "message": "Permission denied."}), 403
    if job_type in ["shorewall:sync"] and current_user.role not in ['admin', 'operator']: return jsonify({"status": "error", "message": "Permission denied."}), 403
    job_id = add_job(job_type, payload, current_user.username); log_event("INFO", f"api:{job_type}", f"Job {job_id} created", username=current_user.username)
...
def update_host_config(host_mac):
    ...
    if not host:
        host = {"mac": host_mac.lower(), "ip_assignment": {"type": "dhcp"}}; config.setdefault("known_hosts", []).append(host)
...
def get_live_online_status():
    config = load_config(); known_hosts_map = {h['mac'].lower(): h for h in config.get("known_hosts", [])}; leases_file = config.get("system_paths", {}).get("dnsmasq_leases_file", ""); active_leases = get_active_leases(leases_file) if leases_file else []
    ips_to_check = {l['ip'] for l in active_leases} | {h.get("ip_assignment", {}).get("ip") for h in known_hosts_map.values() if h.get("ip_assignment", {}).get("ip")}
    online_status_map = check_multiple_hosts_online(list(filter(None, ips_to_check))); final_mac_status = {}
