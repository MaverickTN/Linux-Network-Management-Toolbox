...
def generate_accounting_rules(config: Dict) -> str:
    ...
    for device in active_leases:
        mac = device.get("mac")
        if not mac or mac in processed_macs:
            continue
        ...
        # This simpler loop corrects both the NameError and the formatting bug.
        # It creates two rules per host: one for download (destined FOR the IP)
        # and one for upload (sourced FROM the IP).
...
def generate_policy_rules(config: Dict) -> str:
    ...
    inetctl_rules = [
        "net\t\tall\t\tDROP\t\tinfo",
        "# The following policy MUST be last in this block, or just before your own 'all all' rule",
        "all\t\tall\t\tREJECT\t\tinfo"
    ]
    return "\n".join(inetctl_rules)