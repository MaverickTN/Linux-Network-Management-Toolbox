...
def generate_accounting_rules(config: Dict) -> str:
    """
    Generates the content for /etc/shorewall/accounting using the ACCOUNT action
    for the xtables-addons kernel module.
    """
    ...
    header = [
        ...
        "# Enables per-IP traffic accounting using the xt_ACCOUNT module.",
        "#",
        "# ACTION    SOURCE          DEST"
    ]
    # The ACCOUNT action simply tells the kernel to start counting traffic
    # that matches the source or destination.
    rules = []
    processed_ips = set()
    for device in active_leases:
        ip = device.get("ip")
        if not ip or ip in processed_ips: continue
        
        rules.append(f"ACCOUNT\t{ip}\t\t-") # Uploads from this IP
        rules.append(f"ACCOUNT\t-\t\t{ip}") # Downloads to this IP
