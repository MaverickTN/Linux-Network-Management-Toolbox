...
def generate_accounting_rules(config: Dict) -> str:
    ...
    This version includes the master rules to link accounting to the main chains.
    """
    ...
    # These master rules link the built-in 'download' and 'upload' chains
    # to our custom chains. This is what activates the counting.
    master_rules = [
        "#",
        "# 1. Chain Definitions (one for each host)",
    ...
    for device in active_leases:
        ...
        ip = device.get("ip")
        if not ip: continue
        chain_name = f"acct_{mac.replace(':', '')}"
        device_rules.append(f"{chain_name}\t\t-\t-\t\t{ip}")
        device_rules.append(f"{chain_name}\t\t-\t{ip}\t\t-")
        processed_macs.add(mac)

    linking_rules = [
        "",
        "# 2. Linking Rules (send traffic to the above chains)",
        "# CHAIN              ACTION          SOURCE          DEST",
        # Send all download traffic to be processed by our custom chains
        "download\t\taccounting_bytes\t-\t\t-",
        # Send all upload traffic to be processed by our custom chains
        "upload\t\t\taccounting_bytes\t-\t\t-"
    ]

    return "\n".join(header + master_rules + device_rules + linking_rules) + "\n"
